...
const CalculatorHero = ({ onStart }: CalculatorHeroProps) => {
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col">
      {/* Upper section with canvas effect */}
      <div className="h-[100svh] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-4">
          <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-26 md:h-26 rounded-full overflow-hidden border-2 border-hero-foreground/20 mb-3 sm:mb-4 animate-fade-in-up">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium mb-4 sm:mb-8 animate-fade-in-up">
            ...
            Priser fra <span style={{ color: 'hsl(152 60% 45%)' }}>2.000 kr.</span>
          </div>
          ...
          <div className="animate-fade-in-up flex flex-col items-center" style={{ animationDelay: "0.3s" }}>
            <Button onClick={onStart}
              size="lg"
              className="gradient-primary text-primary-foreground px-8 py-6 text-lg font-semibold rounded-xl glow-primary hover:glow-primary-lg transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]">
              Få din pris
            ...
            <div className="flex flex-col items-center gap-3 mb-6">
              <a href="https://dk.trustpilot.com/review/horizen.dk" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-hero-foreground hover:opacity-80 transition-opacity">
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4].map((i) => (
                    <svg key={i} className="w-4 h-4 text-[#00b67a]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                  ))}
                  <svg className="w-4 h-4 text-[#00b67a]" fill="currentColor" viewBox="0 0 24 24" style={{ clipPath: 'inset(0 40% 0 0)' }}><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                </div>
      ...
      {/* Portfolio section - pure black to match canvas area */}
