...
import { formatDKK, BASE_PRICE } from "@/lib/calculator-config";

const InstantHero = () => {
  useEffect(() => {
    (window as any).dataLayer = (window as any).dataLayer || [];
    (window as any).dataLayer.push({ event: "instant_calculator_view" });
  }, []);
    ...
    <div className="h-[100svh] flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-4">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium mb-4 sm:mb-8 animate-fade-in-up">
          ...
          Fra <span style={{ color: "hsl(152 60% 45%)" }}>{formatDKK(BASE_PRICE)} kr.</span>
          <span className="text-hero-muted">· Gratis hosting i 1 år</span>
        </div>
        ...
        >
          Din professionelle landing page
          <br />
          <span className="text-gradient">til en fast pris</span>
        </h1>
        ...
        >
          Sammensæt din løsning og se prisen med det samme.
        </p>

        <div className="animate-fade-in-up flex flex-col items-center" style={{ animationDelay: "0.3s" }}>
          <button
            onClick={scrollToCalculator}
            className="gradient-primary text-primary-foreground px-8 py-4 text-lg font-semibold rounded-xl glow-primary hover:glow-primary-lg transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
          >
            Se hvad det koster ↓
          </button>
        ...
        <div className="mt-auto pb-4 sm:pb-8 pt-4 sm:pt-8 flex flex-col items-center gap-2 animate-fade-in-up" style={{ animationDelay: "0.6s" }}>
          <span className="text-hero-muted text-xs tracking-wide uppercase">Sammensæt din løsning</span>
          <button onClick={scrollToCalculator} className="animate-bounce">
